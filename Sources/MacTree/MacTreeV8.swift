...
struct MT8Node: Identifiable, Hashable, Sendable {
    let id: Int
    let parentID: Int?
    let name: String
    let path: String
...
actor MT8Scanner {
    ...
    func scan(root: URL, progress: @escaping @Sendable (MT8Progress) async -> Void) async throws -> MT8Snapshot {
        ...
        while let relative = enumerator.nextObject() as? String {
            ...
            if publishCounter >= 35_000 {
                await progress(MT8Progress(items: items, files: files, logical: logical, allocated: allocated,
        ...
        if builders.count > 1 {
            for index in stride(from: builders.count - 1, through: 1, by: -1) {
                ...
                builders[parent].logicalSize = mt8SafeAdd(builders[parent].logicalSize, builders[index].logicalSize)
                builders[parent].allocatedSize = mt8SafeAdd(builders[parent].allocatedSize, builders[index].allocatedSize)
                builders[parent].fileCount = mt8SafeAdd(builders[parent].fileCount, builders[index].fileCount)
            }
        }

        var nodes = builders.map {
            MT8Node(id: $0.id, parentID: $0.parentID, name: $0.name, path: $0.path,
        ...
        return MT8Snapshot(nodes: nodes, rootID: 0, items: items, files: files,
...
@MainActor
final class MT8SearchModel: ObservableObject {
    @Published private(set) var query = ""
    @Published private(set) var resultIDs: [Int] = []
    @Published private(set) var isSearching = false

    private var nodes: [MT8Node] = []
    private var task: Task<Void, Never>?

    func setNodes(_ newNodes: [MT8Node]) {
        task?.cancel()
        nodes = newNodes
        query = ""
        resultIDs = []
    ...
    func search(_ text: String) {
        ...
        task = Task { [weak self] in
            ...
            let ids = await Task.detached(priority: .userInitiated) {
                ...
                return Array(hits.prefix(2000).map(\.id))
            }.value
...
@MainActor
final class MT8Controller: ObservableObject {
    @Published var rootURL = FileManager.default.homeDirectoryForCurrentUser
    @Published var nodes: [MT8Node] = []
    ...
    let search = MT8SearchModel()
    private let scanner = MT8Scanner()
    private var task: Task<Void, Never>?
...
struct MainViewV8: View {
    @Environment(\.scenePhase) private var scenePhase
    @StateObject private var controller = MT8Controller()
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Divider()
            VSplitView {
                MT8TreePane(nodes: controller.nodes,
                ...
                MT8Treemap(nodes: controller.nodes, rootID: controller.rootID, selectedID: $selectedID)
                    .frame(minHeight: 320)
            }
            Divider()
            status
    ...
    private var toolbar: some View {
        HStack(spacing: 10) {
            ...
            Spacer()
            MT8SearchField(model: controller.search)
    ...
    private var summary: some View {
        HStack(spacing: 24) {
            metric("Allocated", mt8Bytes(controller.allocated))
            metric("Logical", mt8Bytes(controller.logical))
    ...
    private var status: some View {
        HStack(spacing: 10) {
            ...
            if let selectedID, controller.nodes.indices.contains(selectedID) {
                ...
                Text("Selected: \(node.path)   \(mt8Bytes(node.allocatedSize))")
...
private struct MT8SearchField: View {
    @ObservedObject var model: MT8SearchModel
...
private struct MT8TreePane: View {
    let nodes: [MT8Node]
    let rootID: Int
    let totalAllocated: UInt64
    let scanVersion: Int
    @ObservedObject var searchModel: MT8SearchModel
...
private struct MT8TreeRow: View {
    let node: MT8Node
    let depth: Int
    let total: UInt64
    let isExpanded: Bool
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            cell(mt8Bytes(node.logicalSize), 105, .trailing)
            cell(mt8Bytes(node.allocatedSize), 105, .trailing)
...
private enum MT8Category: String, CaseIterable {
...
private func mt8DirectCategory(_ node: MT8Node) -> MT8Category {
    let name = node.name.lowercased()
    let path = node.path.lowercased()

    if path.contains("/library/caches/") || name == "cache" || name == "caches" ||
        name == "cacheddata" || name == "code cache" || name == "gpucache" || name.hasSuffix(".cache") {
        return .cache
    }
    if path.contains("/library/logs/") || name == "log" || name == "logs" || name.hasSuffix(".log") {
        return .logs
    }
    if path.contains("/library/preferences/") || name == "preferences" || name == "config" ||
        name == "configs" || name == ".config" || name == "settings" {
        return .config
    }
    if name == "tmp" || name == "temp" || name == "temporary" || path.contains("/tmp/") {
        return .temp
    }
    if path.contains("/steamapps/common/") || path.contains("/games/") || name == "gamedata" || name == "game data" {
        return .gameData
    }
    if path.contains(".app/contents/") || name.hasSuffix(".app") {
        return .application
    }
    if path.contains("/library/application support/") || path.contains("/library/containers/") ||
        ...
        name == "group containers" || name == "saved application state" {
        return .appData
    }
    if path.contains("/developer/") || path.contains("/deriveddata/") || path.contains("/sourcepackages/") ||
        ...
        name == ".gradle" || name == ".swiftpm" || name == ".npm" || name == ".cargo" {
        return .code
    }
    if path.contains("/system/") || path.contains("/library/frameworks/") || name.hasSuffix(".framework") || name == "coreservices" {
        return .system
    }
    if node.isDirectory {
...
private func mt8BestCategory(_ nodeID: Int, _ nodes: [MT8Node]) -> MT8Category {
    guard nodes.indices.contains(nodeID) else { return .other }
    let node = nodes[nodeID]
    let direct = mt8DirectCategory(node)
    if direct != .other { return direct }
    guard node.isDirectory && !node.children.isEmpty else { return .other }

    var weights: [MT8Category: UInt64] = [:]
    for childID in node.children.prefix(40) {
        guard nodes.indices.contains(childID) else { continue }
        let child = nodes[childID]
        var category = mt8DirectCategory(child)
        if category == .other && child.isDirectory {
            for grandID in child.children.prefix(10) {
                ...
                let grandCategory = mt8DirectCategory(grand)
                if grandCategory != .other {
                    weights[grandCategory, default: 0] = mt8SafeAdd(weights[grandCategory, default: 0], grand.allocatedSize)
                }
            }
        } else if category != .other {
            weights[category, default: 0] = mt8SafeAdd(weights[category, default: 0], child.allocatedSize)
        }
    }
    return weights.max(by: { $0.value < $1.value })?.key ?? .other
}

private func mt8CategoryForGroup(_ ids: ArraySlice<Int>, _ nodes: [MT8Node], fallback: MT8Category) -> MT8Category {
    var weights: [MT8Category: UInt64] = [:]
    for id in ids.prefix(80) {
        guard nodes.indices.contains(id) else { continue }
        let category = mt8BestCategory(id, nodes)
        if category != .other {
            weights[category, default: 0] = mt8SafeAdd(weights[category, default: 0], nodes[id].allocatedSize)
        }
    }
...
private enum MT8CellKind { case file, aggregate }

private struct MT8Cell: Identifiable {
    let id: Int
    let nodeID: Int
    let rect: CGRect
    let depth: Int
    let kind: MT8CellKind
    let label: String
    let representedAllocated: UInt64
    let representedFiles: UInt64
    let category: MT8Category
}

private struct MT8Frame: Identifiable {
    let id: Int
    let nodeID: Int
    let rect: CGRect
...
private struct MT8RenderModel {
    ...
    static func empty(_ size: CGSize = .zero) -> MT8RenderModel {
        MT8RenderModel(cells: [], frames: [], buckets: [], cols: 0, rows: 0, size: size)
    }
...
private struct MT8WeightedEntry {
    let token: Int
    let weight: UInt64
}

private struct MT8WeightedLayout {
    func layout(_ entries: [MT8WeightedEntry], in rect: CGRect) -> [(Int, CGRect)] {
        let rect = rect.standardized
        guard mt8RectFinite(rect), rect.width > 0.5, rect.height > 0.5 else { return [] }
    ...
    private func split(_ entries: [MT8WeightedEntry], _ rect: CGRect, _ output: inout [(Int, CGRect)]) {
        guard !entries.isEmpty, mt8RectFinite(rect), rect.width > 0.5, rect.height > 0.5 else { return }
        if entries.count == 1 {
            output.append((entries[0].token, rect))
            return
        }
        let total = entries.reduce(UInt64(0)) { mt8SafeAdd($0, $1.weight) }
        ...
        let leftTotal = left.reduce(UInt64(0)) { mt8SafeAdd($0, $1.weight) }
...
private struct MT8TreemapBuilder {
    let nodes: [MT8Node]
    ...
    func build(rootID: Int, size: CGSize) -> MT8RenderModel {
        ...
        let total = children.reduce(UInt64(0)) { mt8SafeAdd($0, nodes[$1].allocatedSize) }
        guard total > 0 else { return .empty(size) }

        let layout = MT8WeightedLayout()
        let rootRects = layout.layout(children.map { MT8WeightedEntry(token: $0, weight: nodes[$0].allocatedSize) },
                                      in: CGRect(origin: .zero, size: size))
        var cells: [MT8Cell] = []
        var frames: [MT8Frame] = []
        cells.reserveCapacity(maxCells)
        frames.reserveCapacity(500)
    ...
    private func render(id: Int, rect: CGRect, depth: Int, budget: Int,
                        layout: MT8WeightedLayout, cells: inout [MT8Cell], frames: inout [MT8Frame]) {
        let rect = rect.standardized
        guard nodes.indices.contains(id), mt8RectFinite(rect), rect.width > 0.5, rect.height > 0.5 else { return }
        let node = nodes[id]
        let nodeCategory = mt8BestCategory(id, nodes)
        ...
        // Thin headers: preserve hierarchy without eating treemap area.
        ...
        guard mt8RectFinite(content), content.width > 2, content.height > 2 else {
            appendCell(node, rect, depth, .aggregate, node.name, node.allocatedSize, node.fileCount, nodeCategory, &cells)
            return
        }

        if showHeader {
            frames.append(MT8Frame(id: frames.count, nodeID: id, rect: rect,
        ...
        if children.count > keepCount {
            ...
            for childID in remainderSlice {
                remainderWeight = mt8SafeAdd(remainderWeight, nodes[childID].allocatedSize)
                remainderFiles = mt8SafeAdd(remainderFiles, nodes[childID].fileCount)
            }
        }

        var entries = real.map { MT8WeightedEntry(token: $0, weight: nodes[$0].allocatedSize) }
        let remainderToken = -1
        if remainderWeight > 0 { entries.append(MT8WeightedEntry(token: remainderToken, weight: remainderWeight)) }
        ...
        let totalWeight = entries.reduce(UInt64(0)) { mt8SafeAdd($0, $1.weight) }
        ...
        for index in rects.indices {
            ...
            if token == remainderToken {
                ...
                let groupCategory = mt8CategoryForGroup(remainderSlice, nodes, fallback: nodeCategory)
                appendCell(node, rects[index].1, depth + 1, .aggregate, countText,
                           remainderWeight, remainderFiles, groupCategory, &cells)
            } else {
    ...
    private func appendCell(_ node: MT8Node, _ rect: CGRect, _ depth: Int, _ kind: MT8CellKind,
                            _ label: String, _ allocated: UInt64, _ files: UInt64,
                            _ category: MT8Category, _ cells: inout [MT8Cell]) {
        let safe = mt8SafeInset(rect, 0.32)
        guard mt8RectFinite(safe), safe.width > 0.25, safe.height > 0.25 else { return }
        cells.append(MT8Cell(id: cells.count, nodeID: node.id, rect: safe, depth: depth, kind: kind,
                             label: label, representedAllocated: allocated, representedFiles: files,
                             category: category))
    }

    private func makeModel(_ cells: [MT8Cell], _ frames: [MT8Frame], _ size: CGSize) -> MT8RenderModel {
        ...
        for (index, cell) in cells.enumerated() {
            ...
            guard mt8RectFinite(r), r.width > 0, r.height > 0 else { continue }
        ...
        return MT8RenderModel(cells: cells, frames: frames, buckets: buckets,
                              cols: cols, rows: rows, size: size)
    }
}

private struct MT8Treemap: View {
    let nodes: [MT8Node]
    let rootID: Int
    @Binding var selectedID: Int?
        ...
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                ...
                if nodes.indices.contains(rootID) {
                    Text(mt8Bytes(nodes[rootID].allocatedSize)).font(.caption).foregroundStyle(Color.secondary).monospacedDigit()
                }
                Spacer()
                Text("Semantic colors • hover for details")
            ...
            GeometryReader { proxy in
                ...
                let model = MT8TreemapBuilder(nodes: nodes).build(rootID: rootID, size: size)
                MT8Surface(nodes: nodes, model: model, selectedID: $selectedID)
            }
        }
    }
    ...
    private var legend: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 9) {
                ForEach(MT8Category.allCases, id: \.self) { category in
                    HStack(spacing: 3) {
...
private struct MT8Surface: View {
    let nodes: [MT8Node]
    let model: MT8RenderModel
    @Binding var selectedID: Int?
            ...
            ZStack(alignment: .topLeading) {
                Canvas { context, size in
                    ...
                    for cell in model.cells {
                        guard nodes.indices.contains(cell.nodeID), mt8RectFinite(cell.rect) else { continue }
                        ...
                        context.stroke(Path(cell.rect),
                                       with: .color(selectedID == cell.nodeID ? Color.white : Color.black.opacity(0.38)),
                                       lineWidth: selectedID == cell.nodeID ? 1.8 : 0.45)

                        if cell.rect.width > 50 && cell.rect.height > 18 {
                            let maxChars = max(4, Int(cell.rect.width / 6.2))
                            let title = Text(mt8Ellipsize(cell.label, maxCharacters: maxChars))
                            ...
                            if cell.rect.width > 105 && cell.rect.height > 43 {
                                let sub = Text(mt8Bytes(cell.representedAllocated))
                    ...
                    for frame in model.frames {
                        guard nodes.indices.contains(frame.nodeID), mt8RectFinite(frame.rect), mt8RectFinite(frame.headerRect) else { continue }
                        context.fill(Path(frame.headerRect),
                                     with: .color(Color.black.opacity(frame.depth == 0 ? 0.48 : 0.34)))
                        let maxChars = max(5, Int(frame.headerRect.width / 6.0))
                        let label = Text(mt8Ellipsize(nodes[frame.nodeID].name, maxCharacters: maxChars))
                            .font(.system(size: frame.depth <= 1 ? 8.5 : 8, weight: .semibold))
                            .foregroundStyle(Color.white.opacity(0.94))
                        context.draw(label,
                                     at: CGPoint(x: frame.headerRect.minX + 3.5, y: frame.headerRect.minY + 0.5),
                                     anchor: .topLeading)
    ...
    private func hoverCard(_ cell: MT8Cell, _ size: CGSize) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 5) {
            ...
            HStack(spacing: 16) {
                info("Allocated", mt8Bytes(cell.representedAllocated))
                info("Logical", mt8Bytes(node.logicalSize))
                info("Files", cell.representedFiles.formatted())
                info("Type", node.isDirectory ? (cell.label.contains("items") ? "Grouped" : "Folder") : mt8FileTypeLabel(node))
            }
            .font(.caption2)
            Text(node.path)
                .font(.caption2).foregroundStyle(Color.secondary)
...
private func mt8RectFinite(_ rect: CGRect) -> Bool {
    rect.origin.x.isFinite && rect.origin.y.isFinite && rect.size.width.isFinite && rect.size.height.isFinite &&
    rect.width >= 0 && rect.height >= 0
}

private func mt8SafeInset(_ rect: CGRect, _ amount: CGFloat) -> CGRect {
    guard mt8RectFinite(rect) else { return .zero }
    ...
    return mt8RectFinite(result) ? result : rect
}

private func mt8SafeAdd(_ a: UInt64, _ b: UInt64) -> UInt64 {
    let (value, overflow) = a.addingReportingOverflow(b)
    return overflow ? UInt64.max : value
}

private func mt8Ellipsize(_ text: String, maxCharacters: Int) -> String {
...
private func mt8FileTypeLabel(_ node: MT8Node) -> String {
    let ext = (node.name as NSString).pathExtension.lowercased()
    return ext.isEmpty ? "File" : ".\(ext)"
}

private func mt8Bytes(_ value: UInt64) -> String {
